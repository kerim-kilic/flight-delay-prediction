...
one_hot_features <- c("origin", "destination", "carrier")
...
adding_extra_features <- . %>%
  ...
  left_join(., class_delay_pipeline(raw_data) %>% group_by(origin,date) %>% summarise(total_departure_flights=n()),
            by = c("origin"="origin", "date"="date")) %>%
  ### Join the total arrival flights per day per airport
  left_join(., class_delay_pipeline(raw_data) %>% group_by(destination,date) %>% summarise(total_arrival_flights=n()),
  ...
  # ### Join the total departing passengers per hour per day per airport
  # left_join(., class_delay_pipeline(raw_data) %>% group_by(origin,date,planned_departure_local_hour) %>% summarise(total_departing_passengers=sum(seating_capacity)),
  #           by = c("origin"="origin", "date"="date", "planned_departure_local_hour"="planned_departure_local_hour")) %>%
  # ### Join the total arriving passengers per hour per day per airport
  # left_join(., class_delay_pipeline(raw_data) %>% group_by(destination,date,planned_arrival_local_hour) %>% summarise(total_arriving_passengers=sum(seating_capacity)),
  #           by = c("destination"="destination", "date"="date", "planned_arrival_local_hour"="planned_arrival_local_hour")) %>%
  ### Join the weather data of the destination airport
...
classification_split <- create_train_test_split(data = spark_class_pipeline(raw_data),
