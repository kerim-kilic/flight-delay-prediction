...
### Add sum of different types of delays for numerical prediction
...
spark_data <- spark_data %>%
  select(-carrier_delay,-weather_delay,-nas_delay,-security_delay,-late_aircraft_delay,-origin,-destination,-cancelled_flag,-diverted_flag,-num_flights)
...
spark_data_one_hot_encoded <- spark_data
```
...
## Splitted datasets for regression
train_data_regr <- spark_data_one_hot_encoded %>%
  sample_frac(0.008) %>%
  select(-delay)
test_data_regr <- spark_data_one_hot_encoded %>%
  sample_frac(0.002) %>%
  select(-delay)
```
