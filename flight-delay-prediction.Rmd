...
#   summarise(across(c(ws,
#                      # latitude, longitude, elev, 
#                      wd, air_temp, atmos_pres,
#                      visibility, dew_point,precip_6), mean))
...
weather_data <- copy_to(dest = sc,
                       df = weather_data,
                       overwrite = TRUE)
# input_cols_imp <- c("origin_precip_6","origin_ws","origin_wd","origin_air_temp","origin_atmos_pres","origin_visibility","origin_dew_point")
# output_cols_imp <- paste0(input_cols_imp,"_imp")
# weather_data <- ft_imputer(weather_data, input_cols = input_cols_imp, output_cols = output_cols_imp, strategy = "mean")
...
# input_cols_imp <- c("destination_precip_6","destination_ws","destination_wd","destination_air_temp","destination_atmos_pres","destination_visibility","destination_dew_point")
# output_cols_imp <- paste0(input_cols_imp,"_imp")
# weather_data2 <- ft_imputer(weather_data, input_cols = input_cols_imp, output_cols = output_cols_imp, strategy = "mean")
###
...
class_data <- left_join(class_data, 
                        class_delay_pipeline(raw_data) %>% group_by(origin,date) %>% summarise(count1=n()),
                        by=c("origin"="origin","date"="date"))
class_data <- left_join(class_data, 
                        class_delay_pipeline(raw_data) %>% group_by(destination,date) %>% summarise(count2=n()),
                        by=c("destination"="destination","date"="date"))
class_data <- left_join(class_data, 
                        class_delay_pipeline(raw_data) %>% group_by(origin,carrier,date) %>% summarise(count3=n()),
...
class_data <- left_join(class_data, weather_data2,by=c("destination"="destination","date"="date")) %>%
  select(-date)
##########################################################################################################################
# numer_data <- left_join(numer_delay_pipeline(raw_data), weather_data,by=c("origin"="origin","date"="date")) 
# numer_data <- left_join(numer_data, 
#                         numer_delay_pipeline(raw_data) %>% group_by(origin,date) %>% summarise(count1=n()),
#                         by=c("origin"="origin","date"="date"))
# numer_data <- left_join(numer_data, 
#                         numer_delay_pipeline(raw_data) %>% group_by(destination,date) %>% summarise(count2=n()),
#                         by=c("destination"="destination","date"="date"))
# numer_data <- left_join(numer_data, 
#                         numer_delay_pipeline(raw_data) %>% group_by(origin,carrier,date) %>% summarise(count3=n()),
#                         by=c("origin"="origin","carrier"="carrier","date"="date"))
# numer_data <- left_join(numer_data, weather_data2,by=c("destination"="destination","date"="date")) %>%
#   select(-date)
##########################################################################################################################
...
class_data_tmp <- ft_string_indexer(class_data_tmp, input_col = "carrier", output_col = "carrier_ind")
class_data <- ft_one_hot_encoder(class_data_tmp, input_cols = c("origin_ind","destination_ind","carrier_ind"), 
                                 output_cols = c("origin_out","destination_out","carrier_out"))%>%
  select(-origin,-destination,-carrier)
class_data <- ft_vector_assembler(class_data,input_cols = c("origin_out","destination_out","carrier_out"),output_col = "one_hot_output")
### Apply one hot encoding to origin and destination
# numer_data_tmp <- ft_string_indexer(numer_data, input_col = "origin", output_col = "origin_ind")
# numer_data_tmp <- ft_string_indexer(numer_data_tmp, input_col = "destination", output_col = "destination_ind")
# numer_data_tmp <- ft_string_indexer(numer_data_tmp, input_col = "carrier", output_col = "carrier_ind")
# numer_data <- ft_one_hot_encoder(numer_data_tmp, input_cols = c("origin_ind","destination_ind","carrier_ind"), 
#                                  # output_cols = c("origin_out","destination_out","flight_number_out"))
#                                  output_cols = c("origin_out","destination_out","carrier_out"))%>%
#   select(-origin,-destination,-carrier)
# numer_data <- ft_vector_assembler(numer_data,input_cols = c("origin_out","destination_out","carrier_out"),output_col = "one_hot_output")
###
...
#   select(-date)
### Create the train, validation, test split for classification
h2o_classification_split <- create_train_test_split(data = class_data,
                                                    # data = class_delay_pipeline(raw_data),
                                                    sample_size = data_size,
                                                    ratio = 0.6,
...
```{r, dl_metrics_final}
gbm_metrics_final <- generate_metrics_classification(final_dl_model,
                                                    "h2o_classification_train")
gbm_metrics_final %>%
  kbl() %>%
  kable_minimal()
