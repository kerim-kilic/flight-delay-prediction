...
adding_extra_features <- . %>%
  ...
  left_join(., class_delay_pipeline(raw_data) %>% group_by(origin,date,planned_departure_local_hour) %>% summarise(total_departure_flights=n()),
            by = c("origin"="origin", "date"="date", "planned_departure_local_hour"="planned_departure_local_hour")) %>%
  ### Join the total arrival flights per hour per day per airport
  left_join(., class_delay_pipeline(raw_data) %>% group_by(destination,date,planned_arrival_local_hour) %>% summarise(total_arrival_flights=n()),
