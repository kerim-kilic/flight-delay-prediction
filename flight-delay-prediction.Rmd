...
Metrics on the final GBM model using the full train set:

```{r}
gbm_metrics <- model@model$cross_validation_metrics_summary[c(1,2,6,15,16,18,20),]
...
gbm_metrics <- gbm_metrics %>% append(metric_names)
gbm_metrics <- data.frame(gbm_metrics)

gbm_metrics <- gbm_metrics %>%
  select(metric_names, mean) %>%
  mutate(mean = round(mean,3))

colnames(model_metric_results) <- c("metric_names", "value_train")
...
Metrics on using the final GBM model on the predicting with the test set:

```{r}
predictions <- as.data.frame(preds) 

actual_delays <- data.frame(as.data.frame(test_data_h2o) %>% pull(delay))
colnames(actual_delays) <- c("actual_delay")

evaluation_frame <- predictions %>%
  append(actual_delays)

evaluation_frame <- data.frame(evaluation_frame)
  # Calculate the confusion matrix with TP, TN, FP, FN
  
TP <- nrow(evaluation_frame %>%
             filter(actual_delay == "1", 
                    predict == "1"))
  
TN <- nrow(evaluation_frame %>%
             filter(actual_delay == "0", 
                    predict == "0"))
  
FP <- nrow(evaluation_frame %>%
             filter(actual_delay == "0", 
                    predict == "1"))
  
FN <- nrow(evaluation_frame %>%
             filter(actual_delay == "1", 
                    predict == "0"))
  
# Calculate each model metric using confusion matrix
accuracy <- (TP + TN) / (nrow(evaluation_frame))
recall <- (TP) / (TP + FN)
precision <- (TP) / (TP + FP)
missclassification_rate <- (FP + FN) / (nrow(evaluation_frame))
...
colnames(model_metric_results) <- c("metric_names", "value_test")
...
Metrics on the final deep learning model using the full train set:

```{r}
dl_metrics <- final_dl_model@model$cross_validation_metrics_summary[c(1,2,6,15,16,18,20),]
...
dl_metrics <- dl_metrics %>% append(metric_names)
dl_metrics <- data.frame(dl_metrics)

dl_metrics <- dl_metrics %>%
  select(metric_names, mean) %>%
  mutate(mean = round(mean,3))

colnames(dl_metrics) <- c("metric_names", "value_train")
...
Metrics on using the final deep learning model on the predicting with the test set:

```{r}
predictions <- as.data.frame(preds_dl) 
